/** GENERATED: Tue Apr 26 19:36:10 GMT 2022 **/
...
(function(window) {
    var subscriptions = {}, cache = {}, we, ops = Object.prototype.toString, aps = Array.prototype.slice;
    ...
    we.subscribe = function(topic) {
        var l, i, options, callback, iCache, args = aps.call(arguments, 1);
...
(function($, window) {
    var defaults = {
        ...
        escKey: true,
        arrowKey: true
    }, colorbox = "colorbox", prefix = "cbox", event_open = prefix + "_open", event_load = prefix + "_load", event_complete = prefix + "_complete", event_cleanup = prefix + "_cleanup", event_closed = prefix + "_closed", event_purge = prefix + "_purge", event_loaded = prefix + "_loaded", isIE = $.browser.msie && !$.support.opacity, isIE6 = isIE && $.browser.version < 7, event_ie6 = prefix + "_IE6", $overlay, $box, $wrap, $content, $topBorder, $leftBorder, $rightBorder, $bottomBorder, $related, $window, $loaded, $loadingBay, $loadingOverlay, $title, $current, $slideshow, $next, $prev, $close, interfaceHeight, interfaceWidth, loadedHeight, loadedWidth, element, index, settings, open, active, closing = false, publicMethod, boxElement = prefix + "Element";
    function $div(id, css) {
        id = id ? ' id="' + prefix + id + '"' : "";
        css = css ? ' style="' + css + '"' : "";
        return $("<div" + id + css + "/>");
...
(function(jQuery, window, document, undefined) {
    ...
    window.newWin = function(url) {
        var sNewWin = "newWin", options = "width=800,height=525,resizable=yes,scrollbars=yes,location=yes,status=yes,toolbar=yes,menubar=yes,directories=yes", subWindow, w = window;
    ...
    window.reducePresBy = function() {
        var $presby = jQuery("#sub-branding .presby, .mod-feature-header .presby");
        $presby.css({
            width: presByOriginalWidth + "px"
        });
    }
    ;
    window.rIF = function(ht) {
        document.getElementById("spFrame").height = parseInt(ht) + 40;
    }
    ;
    window.gotosite = function(l) {
        if (!!l) {
            location.href = l;
        }
    }
    ...
    jQuery.cookie = function(key, value, options) {
        var settings = jQuery.extend({
            ...
            expires: null
        }, options), regex, r, date, expires, path = settings.path !== null ? "; path=" + settings.path : "", domain = settings.domain !== null ? "; domain=" + settings.domain : "", secure = settings.path === true ? "; secure=" : "";
    ...
    espn.sport.is = function(id) {
        var sportId = espn.sport.id()
          , idType = typeof sportId;
    ...
    (function($) {
        ...
        function findAccordions(el) {
            var a = (el) ? $(el) : $("div.mod-accordion").not(".ui-accordion"), ac = "active", activate = function(e) {
                $(e.target).parent().siblings("a.active").removeClass(ac).end().end().addClass(ac);
            ...
            if (len) {
                while (len--) {
                    ...
                    $("a.mod-sub-accordion-trigger", acc).click(function(e) {
                        activate(e);
                    });
                }
                a.accordion({
                ...
                $("a.mod-accordion-trigger").click(function(e) {
                    activate(e);
                });
            }
        }
        ...
        function initFlashSubHeader(section) {
            if (window.Jiffy) {
                Jiffy.mark("initFlashSubheaderStart");
            }
            var title, indexUrl, data, gh, flashVars, params, attributes, suspendRender, useNonFlashHeader, cufon_load_only_once = true;
            if (section.length) {
            ...
            if (window.Jiffy) {
                Jiffy.measure("initFlashSubheaderDone", "initFlashSubheaderStart");
            }
        }
        ...
        function initTabs(sel, tabSettings) {
            ...
            if (window.Jiffy) {
                Jiffy.mark("initTabsStart");
            }
            tabContainer.data("initialized", true);
            tabContainer.tabs(settings);
            if (window.Jiffy) {
                Jiffy.measure("initTabsEnd", "initTabsStart");
            }
        }
        ...
        function init() {
            ...
            if (window.Jiffy) {
                Jiffy.measure("onDOMReady::Complete", "onDOMReady");
            }
        }
        $(function() {
            init();
